...
-- their department number, their
-- start date, their end date, and a new column 
-- 'is_current_employee' that is a
-- 1 if the employee is still with the company 
-- and 0 if not.
...
-- new column 'alpha_group' that 
-- returns 'A-H', 'I-Q', or 'R-Z' depending on the
-- first letter of their last name.

-- LEFT is a function that does what you may suspect!
SELECT
...
-- How many employees (current or previous) 
-- were born in each decade?

SELECT
    COUNT(
		CASE WHEN 
        birth_date BETWEEN '1950-01-01' AND '1959-12-31' 
			THEN birth_date ELSE NULL END) AS '50s'
    COUNT(CASE WHEN birth_date BETWEEN '1960-01-01' 
		AND '1969-12-31' 
        THEN birth_date ELSE NULL END) AS '60s'
FROM employees;
